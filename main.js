...
async function selectIpodFolder() {
    try {
        if (!('showDirectoryPicker' in window)) {
            log('Directory picker API is unavailable in this browser.', 'error');
            log('Brave users: open brave://flags/#file-system-access-api, enable the flag, relaunch Brave, then reload this page.', 'info');
...
async function setupFirewireGuid() {
    try {
        if (!('usb' in navigator)) {
            log('WebUSB is not available in this browser. Use a Chromium browser with USB access enabled.', 'error');
            return;
        }
...
function syncTrackListHeightToPlaylistList() {
    const playlistList = document.getElementById('playlistList');
    const trackTableContainer = document.getElementById('trackTableContainer');
    if (!playlistList || !trackTableContainer) return;

    const playlistHeight = Math.round(playlistList.getBoundingClientRect().height);
    if (!Number.isFinite(playlistHeight) || playlistHeight <= 0) return;

    trackTableContainer.style.maxHeight = `${playlistHeight}px`;
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    log(`Capabilities: secureContext=${browserCapabilities.secureContext}, directoryPicker=${browserCapabilities.directoryPicker}, openFilePicker=${browserCapabilities.openFilePicker}, webUsb=${browserCapabilities.webUsb}`, 'info');
    if (!browserCapabilities.directoryPicker) {
        log('Directory picker API unavailable. Connect will require browser compatibility setup.', 'warning');
    }
    if (!browserCapabilities.openFilePicker) {
        log('Open file picker API unavailable. Upload button may not work; directory upload and drag/drop can still be used.', 'warning');
    }
    if (!browserCapabilities.webUsb) {
        log('WebUSB API unavailable. Automatic FirewireGuid setup may be unavailable.', 'warning');
    }
    ...
    initDragAndDrop();
    syncTrackListHeightToPlaylistList();
    ...
    const playlistList = document.getElementById('playlistList');
    if (playlistList && 'ResizeObserver' in window) {
        const ro = new ResizeObserver(() => syncTrackListHeightToPlaylistList());
        ro.observe(playlistList);
    }
    window.addEventListener('resize', syncTrackListHeightToPlaylistList);
